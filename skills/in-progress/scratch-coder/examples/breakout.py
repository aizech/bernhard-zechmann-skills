...
def create_breakout():
    ...
    for r in range(ROWS):
        for c in range(COLS):
            ...
            bk = project.createSprite(
                f"Brick{r}{c}", x=bx, y=by, rotation_style="don't rotate"
            )
    ...
    paddle.createScript(
        ...
        Forever(
            If(
                ...
                If(GreaterThan(XPosition(), -175), ChangeX(-10)),
            ),
            If(KeyPressed("right arrow"), If(LessThan(XPosition(), 175), ChangeX(10))),
            Wait(0.02),
        ),
    )
    ...
    ball.createScript(
        ...
        Forever(
            ...
            If(
                And(TouchingObject(paddle), LessThan(ball_dy, 0)),
                SetVariable(ball_dy, 1),
                Play("paddle"),
            ),
            ...
            If(GreaterThan(score, 239), SetVariable(game_over, 2), Stop(THIS_SCRIPT)),
            # Ball fell below paddle — lose a life.
            # Scratch fence clamps at y=-180, so use -172 as threshold.
            If(
                LessThan(YPosition(), -172),
                ChangeVariable(lives, -1),
                If(
                    LessThan(lives, 1), SetVariable(game_over, 1), Stop(THIS_SCRIPT)
                ).Else(
                    GoToPosition(0, -80),
                    SetVariable(ball_dx, 1),
                    SetVariable(ball_dy, 1),
    ...
    score_hud.createScript(
        ...
        SetSize(5),
        Forever(
            Say(Join(Join("Score: ", score), Join("   Lives: ", lives))), Wait(0.1)
        ),
    )
