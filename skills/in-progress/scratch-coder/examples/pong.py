...
def create_pong():
    ...
    stage.createScript(
        WhenFlagClicked(), SetVariable(player_score, 0), SetVariable(ai_score, 0)
    )
    ...
    player.createScript(
        ...
        Show(),
        Forever(
            If(KeyPressed("w"), ChangeY(15)),
            If(KeyPressed("s"), ChangeY(-15)),
    ...
    ball.createScript(
        ...
        Forever(
            ...
            If(TouchingObject(player), PointInDirection(PickRandom(60, 120))),
            # Hit AI paddle — deflect toward player (LEFT = 240-300°)
            If(TouchingObject(ai), PointInDirection(PickRandom(240, 300))),
        ),
    )
    ...
    ai.createScript(
        ...
        Forever(
            If(
                GreaterThan(GetAttribute(Y_POSITION, ball), Add(YPosition(), 10)),
                ChangeY(6),
            ),
            If(
                LessThan(GetAttribute(Y_POSITION, ball), Subtract(YPosition(), 10)),
                ChangeY(-6),
            ),
            If(GreaterThan(YPosition(), 150), SetY(150)),
            If(LessThan(YPosition(), -150), SetY(-150)),
    ...
    score_right.createScript(
        ...
        GoToPosition(170, 130),
        SetSize(5),
        Forever(Say(Join("AI: ", ai_score)), Wait(0.1)),
    )
