...
BUILT_IN_COSTUMES = {
    ...
    "empty": [
        {
            "assetId": "1fab14b135a4262c64eaf6f5009f326e",
            "name": "costume1",
            "md5ext": "1fab14b135a4262c64eaf6f5009f326e.svg",
            "dataFormat": "svg",
            "bitmapResolution": 1,
            "rotationCenterX": 0,
            "rotationCenterY": 0,
    ...
    "tennis": [
        {
            "assetId": "19307039b0315b9d74609765c99780bc",
            "name": "tennis ball",
            "md5ext": "19307039b0315b9d74609765c99780bc.svg",
            "dataFormat": "svg",
            "bitmapResolution": 1,
            "rotationCenterX": 22,
            "rotationCenterY": 22,
    ...
    "basketball": [
        {
            "assetId": "30a2740de4fc5426a5a1f3b04d537f52",
            "name": "basketball",
            "md5ext": "30a2740de4fc5426a5a1f3b04d537f52.svg",
            "dataFormat": "svg",
            "bitmapResolution": 1,
            "rotationCenterX": 25,
            "rotationCenterY": 25,
...
def resolve_backdrop(backdrop_spec: str | dict) -> dict:
    ...
    if isinstance(backdrop_spec, str):
        return BUILT_IN_COSTUMES.get(backdrop_spec, BUILT_IN_COSTUMES["empty"]).copy()
    elif isinstance(backdrop_spec, dict):
        if "costume" in backdrop_spec:
            return BUILT_IN_COSTUMES.get(
                backdrop_spec["costume"], BUILT_IN_COSTUMES["empty"]
            ).copy()
        if "assetId" in backdrop_spec or "md5ext" in backdrop_spec:
            return backdrop_spec
        if "color" in backdrop_spec:
            return backdrop_spec
        return BUILT_IN_COSTUMES.get("empty", {}).copy()
    return BUILT_IN_COSTUMES["empty"].copy()
...
def create_sprite(
    ...
    if blocks and isinstance(blocks, dict):
        has_string_keys = any(
            isinstance(k, str) and not k.startswith("_") for k in blocks.keys()
        )
...
def download_asset(md5ext: str, cache_dir: Path) -> bytes | None:
    """Download an asset from Scratch CDN or return cached version."""
...
def main():
    ...
    print(
        f"Targets: {len(project['targets'])} (1 stage + {len(project['targets']) - 1} sprites)"
    )
