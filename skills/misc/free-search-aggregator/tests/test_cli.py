...
class TestCLI(unittest.TestCase):
    ...
    def test_brave_search_compat_mode(self) -> None:
        ...
        mock_search.assert_called_once_with(
            "milan events", max_results=8, config_path=None
        )
        self.assertEqual(
            out.getvalue().strip(), json.dumps(payload, ensure_ascii=False)
        )

    def test_router_error_exit_two(self) -> None:
        with patch(
            "free_search.__main__.search", side_effect=SearchRouterError("all failed")
        ):
            err = io.StringIO()
    ...
    def test_status_shows_quota_without_search(self) -> None:
        ...
        with patch(
            "free_search.__main__.get_quota_status", return_value=payload
        ) as mock_status:
    ...
    def test_status_reset_calls_reset_quota(self) -> None:
        ...
        with patch(
            "free_search.__main__.reset_quota", return_value=payload
        ) as mock_reset:
            out = io.StringIO()
            with redirect_stdout(out):
