...
class BraveProvider(BaseProvider):
    ...
    def search(self, query: str, *, max_results: int) -> list[SearchItem]:
        ...
        for idx, row in enumerate(raw_results[:max_results], start=1):
            ...
            items.append(
                SearchItem(
                    title=title, url=url, snippet=snippet, source=self.name, rank=idx
                )
            )
...
class DuckDuckGoProvider(BaseProvider):
    ...
    def search(self, query: str, *, max_results: int) -> list[SearchItem]:
        ...
        try:
            logger.debug(
                "DuckDuckGo HTML request: query=%r max_results=%s", query, max_results
            )
            response = self.session.get(
        ...
        for node in nodes:
            ...
            items.append(
                SearchItem(
                    title=title, url=href, snippet=snippet, source=self.name, rank=rank
                )
            )
            if rank >= max_results:
                break
        ...
        if not items:
            ...
            logger.info(
                "DuckDuckGo HTML returned %s results for query=%r", len(items), query
            )
        return items
...
class SearchApiProvider(BaseProvider):
    ...
    def search(self, query: str, *, max_results: int) -> list[SearchItem]:
        ...
        try:
            logger.debug(
                "SearchApi request: query=%r max_results=%s", query, max_results
            )
            response = self.session.get(
                self.endpoint, params=params, headers=headers, timeout=self.timeout
            )
...
class DuckDuckGoInstantProvider(BaseProvider):
    ...
    def search(self, query: str, *, max_results: int) -> list[SearchItem]:
        ...
        try:
            logger.debug(
                "DuckDuckGo Instant request: query=%r max_results=%s",
                query,
                max_results,
            )
            response = self.session.get(
        ...
        logger.info(
            "DuckDuckGo Instant returned %s results for query=%r", len(items), query
        )
        return items
...
class BingRSSProvider(BaseProvider):
    ...
    def search(self, query: str, *, max_results: int) -> list[SearchItem]:
        ...
        try:
            logger.debug(
                "Bing RSS request: query=%r max_results=%s", query, max_results
            )
            response = self.session.get(
                self.endpoint, params=params, headers=headers, timeout=self.timeout
            )
...
class MojeekProvider(BaseProvider):
    ...
    def _search_api(
        ...
        try:
            logger.debug(
                "Mojeek API request: query=%r max_results=%s", query, max_results
            )
            response = self.session.get(
    ...
    def _search_html(self, query: str, *, max_results: int) -> list[SearchItem]:
        ...
        try:
            logger.debug(
                "Mojeek HTML request: query=%r max_results=%s", query, max_results
            )
            response = self.session.get(
                self.endpoint, params=params, headers=headers, timeout=self.timeout
            )
        ...
        for li in soup.select("ul.results-standard li"):
            ...
            items.append(
                SearchItem(
                    title=title, url=url, snippet=snippet, source=self.name, rank=rank
                )
            )
            if rank >= max_results:
                break
        ...
        if not items:
            ...
            logger.info(
                "Mojeek HTML returned %s results for query=%r", len(items), query
            )
        return items
...
class SearXNGProvider(BaseProvider):
    ...
    def search(self, query: str, *, max_results: int) -> list[SearchItem]:
        ...
        if not base:
            raise AuthError(
                "SearXNG endpoint not configured (set searxng.endpoint in providers.yaml)"
            )
        ...
        if response.status_code in (403, 401):
            raise UpstreamError(
                f"SearXNG returned {response.status_code} — JSON API may be disabled on this instance"
            )
        if response.status_code >= 500:
            raise UpstreamError(f"SearXNG server error: HTTP {response.status_code}")
        if response.status_code >= 400:
            raise UpstreamError(
                f"SearXNG request rejected: HTTP {response.status_code}"
            )
        ...
        try:
            ...
            raise ParseError(
                "SearXNG returned non-JSON (enable format=json on your instance)"
            ) from exc
        ...
        for row in raw_results:
            ...
            items.append(
                SearchItem(
                    title=title, url=url_, snippet=snippet, source=self.name, rank=rank
                )
            )
            if rank >= max_results:
                break
        ...
        if not items:
            logger.info(
                "SearXNG returned no results for query=%r (endpoint=%s)", query, base
            )
        else:
...
class WikipediaProvider(BaseProvider):
    ...
    def search(self, query: str, *, max_results: int) -> list[SearchItem]:
        ...
        try:
            logger.debug(
                "Wikipedia request: query=%r max_results=%s lang=%s",
                query,
                max_results,
                lang,
            )
        ...
        if response.status_code >= 400:
            raise UpstreamError(
                f"Wikipedia request rejected: HTTP {response.status_code}"
            )
    ...
    def search(self, query: str, *, max_results: int) -> list[SearchItem]:
        ...
        if response.status_code == 429:
            raise RateLimitError(
                "Google CSE rate limited (daily quota likely exhausted)"
            )
        if response.status_code >= 500:
...
class ExaProvider(BaseProvider):
    ...
    def search(self, query: str, *, max_results: int) -> list[SearchItem]:
        ...
        try:
            logger.debug(
                "Exa request: query=%r max_results=%s type=%s",
                query,
                max_results,
                body["type"],
            )
...
class BaiduProvider(BaseProvider):
    ...
    def search(self, query: str, *, max_results: int) -> list[SearchItem]:
        ...
        if isinstance(payload, dict) and "code" in payload:
            raise UpstreamError(
                f"Baidu API error: {payload.get('message', 'unknown error')}"
            )
