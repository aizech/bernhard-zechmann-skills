...
def _run_doctor(*, config_path: str | None = None) -> dict[str, Any]:
        ...
        checks.append(
            {
                "check": "config_load",
                "status": "ok",
                "providers_configured": len(router.order),
            }
        )
        ...
        if name in _ENV_KEY_MAP:
            ...
            if missing:
                provider_status.append(
                    {"provider": name, "status": "missing_keys", "keys": missing}
                )
                continue
    ...
    try:
        ...
        high_usage = [
            p["provider"]
            for p in quota.get("providers", [])
            if isinstance(p.get("percentage_used"), (int, float))
            and p["percentage_used"] >= 80
        ]
        checks.append(
            {
                "check": "quota",
                "status": "warning" if high_usage else "ok",
...
def _run_setup(*, config_path: str | None = None) -> dict[str, Any]:
    ...
    if needs_setup > 0:
        recommendations.append(
            f"{needs_setup} providers need API keys — set the environment variables to enable them"
        )
    if ready < 4:
        recommendations.append(
            "Consider setting up more API-key providers for better failover coverage"
        )
    recommendations.append(
        "Run 'python -m free_search doctor' for a full system diagnostic"
    )
    recommendations.append(
        "Run 'python -m free_search discover' to find additional search sources"
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if normalized_args and normalized_args[0].lower() in (
        "status",
        "remaining",
        "quota",
    ):
        parser = argparse.ArgumentParser(
            description="Show or reset provider quota usage"
        )
        ...
        parser.add_argument(
            "--reset",
            action="store_true",
            help="Reset quota counters before showing status",
        )
        parser.add_argument(
            "--real",
            action="store_true",
            help="Fetch real provider quota when supported",
        )
        parser.add_argument(
            "--probe",
            action="store_true",
            help="Allow probe requests for providers without quota endpoints (may consume quota)",
        )
        ...
        try:
            ...
            print(
                json.dumps({"error": f"unexpected_error: {exc}"}, ensure_ascii=False),
                file=sys.stderr,
            )
            return 1
    ...
    if normalized_args and normalized_args[0].lower() == "gc":
        parser = argparse.ArgumentParser(
            description="Clean old search cache/report files"
        )
    ...
    if normalized_args and normalized_args[0].lower() == "task":
        parser = argparse.ArgumentParser(
            description="Run task-level multi-query search"
        )
    ...
    if normalized_args and normalized_args[0].lower() == "health":
        ...
        parser.add_argument("--compact", action="store_true", help="Print compact JSON")
        parser.add_argument(
            "--compact-health", action="store_true", help="Compact old health records"
        )
        args = parser.parse_args(normalized_args[1:])
    ...
    if normalized_args and normalized_args[0].lower() == "discover":
        parser = argparse.ArgumentParser(
            description="Discover and probe search sources"
        )
        parser.add_argument("--config", default=None, help="Path to providers.yaml")
        parser.add_argument("--compact", action="store_true", help="Print compact JSON")
        parser.add_argument("--log-level", default="INFO")
    ...
    if normalized_args and normalized_args[0].lower() == "setup":
        parser = argparse.ArgumentParser(
            description="Show setup status and recommendations"
        )
        parser.add_argument("--config", default=None, help="Path to providers.yaml")
        parser.add_argument("--compact", action="store_true", help="Print compact JSON")
        args = parser.parse_args(normalized_args[1:])
