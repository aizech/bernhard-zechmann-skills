        ...
        try:
            ...
            if source_type == "searxng":
                ...
                if result_path is None:
                    ...
                    results = (
                        data.get(result_path, []) if isinstance(data, dict) else []
                    )
...
class SourceDiscovery:
    ...
    def probe_all(self) -> dict[str, Any]:
        ...
        results.sort(
            key=lambda x: (0 if x["status"] == "ok" else 1, x.get("latency_ms", 9999))
        )
        return {
    ...
    def run_discovery(self) -> dict[str, Any]:
        ...
        degraded: list[str] = []
        for p in existing.get("configured_providers", []):
            if p["status"] in ("error", "empty") and p["provider"] not in (
                "searxng",
                "yacy",
            ):
        ...
        try:
            with self.storage_path.open("a", encoding="utf-8") as f:
                entry = {
                    ...
                    "existing_ok": sum(
                        1
                        for p in result.get("existing_providers", [])
                        if p.get("status") == "ok"
                    ),
                    "existing_total": len(result.get("existing_providers", [])),
                    "candidates_available": sum(
                        1
                        for c in result.get("candidate_results", [])
