...
def _extract_candidates(text: str) -> list[str]:
    ...
    for part in raw:
        ...
        if not part:
            continue
        tokens = [
            t for t in re.split(r"\s+", part) if t and t.lower() not in _STOPWORDS
        ]
        if not tokens:
            continue
...
def generate_task_queries(task: str, *, max_queries: int = 6) -> list[str]:
    ...
    if (
        any(k in core_task for k in ["是什么"])
        or "what is" in lower
        or "what's" in lower
    ):
...
def task_search(
    ...
    *,
    ...
    with ThreadPoolExecutor(max_workers=workers) as pool:
        futs = [
            pool.submit(
                _run_one_query, router, q, max_results_per_query=max_results_per_query
            )
            for q in queries
        ]
    ...
    for g in grouped_results:
        ...
        for item in results:
            ...
            merged.append(item)
            if (
                isinstance(max_merged_results, int)
                and max_merged_results > 0
                and len(merged) >= max_merged_results
            ):
                break
        if (
            isinstance(max_merged_results, int)
