...
class HealthTracker:
    ...
    def record(
        ...
        *,
        ...
        try:
            ...
            logger.warning(
                "Failed to write health record for %s", provider, exc_info=True
            )

    def _load_records(
        self, window_hours: int = _DEFAULT_WINDOW_HOURS
    ) -> list[dict[str, Any]]:
    ...
    def get_scores(self, window_hours: int = _DEFAULT_WINDOW_HOURS) -> dict[str, float]:
        ...
        for name, recs in by_provider.items():
            ...
            avg_latency = (
                sum(latencies) / len(latencies) if latencies else _LATENCY_SLOW_MS
            )
            ...
            if avg_latency <= _LATENCY_FAST_MS:
                ...
                latency_score = 1.0 - (avg_latency - _LATENCY_FAST_MS) / (
                    _LATENCY_SLOW_MS - _LATENCY_FAST_MS
                )
    ...
    def smart_order(self, base_order: list[str]) -> list[str]:
        ...
        total_records = sum(1 for _ in self._load_records() if True)
