...
class QuotaState:
    ...
    def save(self) -> None:
        try:
            ...
            logger.warning(
                "Failed to persist quota state at %s", self.path, exc_info=True
            )
...
class SearchRouter:
    ...
    def __init__(self, config_path: str | os.PathLike[str] | None = None) -> None:
        ...
        quota_path = self.config.get("router", {}).get(
    ...
    def search(self, query: str, *, max_results: int = 8) -> dict[str, Any]:
        ...
        for name in self.order:
            ...
            if not provider:
                attempted.append(
                    {"provider": name, "status": "skipped", "reason": "not_configured"}
                )
                continue
            if not provider.is_enabled():
                attempted.append(
                    {"provider": name, "status": "skipped", "reason": "disabled"}
                )
                continue
            if not self._has_quota(name, cfg):
                attempted.append(
                    {"provider": name, "status": "failed", "reason": "quota_exceeded"}
                )
                continue

            logger.info("Attempting provider=%s for query=%r", name, query)
            items, attempt, count_quota = self._try_provider(
                name, provider, query, max_results
            )
        ...
        raise SearchRouterError(
            ...
            + " | ".join(
                f"{x['provider']}:{x.get('reason', 'unknown')}" for x in attempted
            )
        )
