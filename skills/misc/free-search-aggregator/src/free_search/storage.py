...
def _build_report_markdown(
    query: str, payload: dict[str, Any], *, mode: str, ts: datetime
) -> str:
...
def persist_search_payload(
    *, query: str, payload: dict[str, Any], mode: str
) -> dict[str, str]:
    ts = _now()
    ...
    digest = hashlib.sha256(
        f"{mode}|{query}|{ts.timestamp()}".encode("utf-8")
    ).hexdigest()[:10]
    slug = _slug(query)
    ...
    report_path.write_text(
        _build_report_markdown(query, payload, mode=mode, ts=ts), encoding="utf-8"
    )
